...
function _summarizeSuitPriorities(gs, botId) {
  const botTeam = getPlayerTeam(gs, botId);
  const teammates = getTeamPlayers(gs, botTeam).filter((p) => p.playerId !== botId);
  const teamPlayers = [{ playerId: botId }, ...teammates];
  const opponents = gs.players.filter((p) => getPlayerTeam(gs, p.playerId) !== botTeam);
  ...
  const cardToHalfSuit = buildCardToHalfSuitMap(gs.variant);
  const botHalfSuits = new Set();
  for (const card of getHand(gs, botId)) {
    const halfSuitId = cardToHalfSuit.get(card);
    if (halfSuitId && !isHalfSuitDeclared(gs, halfSuitId)) {
      botHalfSuits.add(halfSuitId);
    }
  }

  return [...botHalfSuits].map((halfSuitId) => {
    const teamCount = Math.max(
      _getVisibleTeamHalfSuitCount(gs, botId, teamPlayers, halfSuitId),
      _getVisibleTeamHalfSuitMinimumCount(gs, botId, teamPlayers, halfSuitId)
    );
    const opponentKnownCount = _getVisibleOpponentHalfSuitCount(gs, botId, teamPlayers, halfSuitId);
    const teammateAssistPriority = _getTeammateAssistPriority(
      gs,
      botId,
      botTeam,
      halfSuitId,
    ...
    return {
      ...
      teamCount,
      opponentKnownCount,
      teammateAssistPriority,
      closeoutPriority: _getSuitCloseoutPriority(teamCount),
      teamSignalStrength: _getTeamSignalStrength(gs, botTeam, halfSuitId, currentMoveIndex),
      opponentSignalThreat: _getTeamSignalStrength(
        gs,
        botTeam === 1 ? 2 : 1,
        halfSuitId,
        currentMoveIndex
      ),
      liveOpponents: opponents.filter((p) => getCardCount(gs, p.playerId) > 0).length,
    };
  }).sort((a, b) => {
    const assistDiff = b.teammateAssistPriority - a.teammateAssistPriority;
    if (assistDiff !== 0) return assistDiff;
    const closeoutDiff = b.closeoutPriority - a.closeoutPriority;
    if (closeoutDiff !== 0) return closeoutDiff;
    const threatDiff = b.opponentKnownCount - a.opponentKnownCount;
    if (threatDiff !== 0) return threatDiff;
    return a.halfSuitId.localeCompare(b.halfSuitId);
  });
}

function _summarizeKnownCardState(gs, botId, cardId) {
  if (!cardId) {
    return {
      knownHolderCount: 0,
      knownMissingCount: 0,
      unresolvedCount: 0,
      knownHolderTeam: null,
    };
  }

  let knownHolderCount = 0;
  let knownMissingCount = 0;
  let unresolvedCount = 0;

  for (const player of gs.players) {
    const known = _getEffectiveKnowledge(gs, botId, player.playerId, cardId);
    if (known === true) knownHolderCount += 1;
    else if (known === false) knownMissingCount += 1;
    else unresolvedCount += 1;
  }

  const knownHolderId = _getVisibleKnownHolder(gs, botId, cardId);
  const knownHolderTeam = knownHolderId && knownHolderId !== '__conflict__'
    ? getPlayerTeam(gs, knownHolderId)
    : null;
...
function _summarizeHalfSuitKnowledge(gs, botId, halfSuitId) {
  if (!halfSuitId) {
    return {
      totalCardsInSuit: 0,
      knownOnTeamCount: 0,
      knownOnOpponentCount: 0,
      unresolvedCount: 0,
      opponentProfiles: [],
    };
  }
  ...
  let knownOnTeamCount = 0;
  let knownOnOpponentCount = 0;
  let unresolvedCount = 0;

  for (const cardId of cards) {
    const knownTeam = _getVisibleKnownTeam(gs, botId, cardId);
    if (knownTeam === botTeam) knownOnTeamCount += 1;
    else if (knownTeam && knownTeam !== botTeam) knownOnOpponentCount += 1;
    else unresolvedCount += 1;
  }

  const blockedOpponentIds = _getBlockedOpponentIds(gs, botId, currentMoveIndex);
  const opponents = gs.players.filter((p) => getPlayerTeam(gs, p.playerId) !== botTeam);
  const opponentProfiles = opponents.map((opponent) => ({
    knownSuitCards: _getVisibleKnownHalfSuitCardCount(gs, botId, opponent.playerId, halfSuitId),
    recentActivity: _getRecentHalfSuitActivityStrength(gs, opponent.playerId, halfSuitId, currentMoveIndex),
    capacity: _getVisibleTargetCapacity(gs, botId, opponent.playerId, halfSuitId),
    presence: _hasHalfSuitPresence(gs, opponent.playerId, halfSuitId) ? 1 : 0,
    blockedByTeammateChase: blockedOpponentIds.has(opponent.playerId) ? 1 : 0,
    liveCards: getCardCount(gs, opponent.playerId),
  })).sort((a, b) => _compareOpponentAskPriority(a, b));

  return {
    totalCardsInSuit: cards.length,
    knownOnTeamCount,
    knownOnOpponentCount,
    unresolvedCount,
    opponentProfiles,
  };
}

function _buildReasonEvidence(reason, selectedSuitPriority, selectedHalfSuitKnowledge, knownCardState, context) {
  if (reason === 'known_holder') {
    return {
      knownCardState,
      selectedSuitPriority,
    };
  }

  if (reason === 'teammate_signal_followup') {
    return {
      selectedSuitPriority,
      blockedTeammateChases: context.blockedTeammateChaseCount,
      liveTeamSignalSuits: context.liveTeamSignalSuits,
    };
  }

  if (reason === 'closeout_push') {
    return {
      closeoutPriority: selectedSuitPriority?.closeoutPriority ?? 0,
      teamCount: selectedSuitPriority?.teamCount ?? 0,
      opponentKnownCount: selectedSuitPriority?.opponentKnownCount ?? 0,
      selectedHalfSuitKnowledge,
    };
  }

  if (reason === 'priority_guess') {
    return {
      selectedSuitPriority,
      selectedHalfSuitKnowledge,
      topSuitPriorityCount: context.topSuitPriorityCount,
    };
  }

  if (reason === 'signal_probe') {
    return {
      teamSignalStrength: selectedSuitPriority?.teamSignalStrength ?? 0,
      opponentSignalThreat: selectedSuitPriority?.opponentSignalThreat ?? 0,
      selectedHalfSuitKnowledge,
    };
  }

  if (reason === 'emergency_guess') {
    return {
      validOpponentCount: context.validOpponentCount,
      selectedHalfSuitKnowledge,
      note: 'normal filtered search exhausted legal asks',
    };
  }
...
function buildBotDecisionThinkingSnapshot(gs, botId, decision) {
  const botTeam = getPlayerTeam(gs, botId);
  const opponents = gs.players.filter((p) => getPlayerTeam(gs, p.playerId) !== botTeam);
  const validOpponents = opponents.filter((p) => getCardCount(gs, p.playerId) > 0);
  const undeclaredHalfSuits = allHalfSuitIds().filter((id) => !isHalfSuitDeclared(gs, id));
  ...
  const suitPriorities = _summarizeSuitPriorities(gs, botId);
  const topSuitPriorities = suitPriorities.slice(0, 3);
  const blockedChases = _getBlockingTeammateChases(gs, botId, currentMoveIndex)
    .slice(0, 3)
    .map((chase) => ({
      halfSuitId: chase.halfSuitId,
      teammateMinimum: chase.teammateMinimum,
      signalStrength: chase.signalStrength,
      blockedOpponentCount: chase.blockedOpponentIds.length,
    }));

  const selectedHalfSuitId = decision?.action === 'ask'
    ? cardHalfSuit(gs, decision.cardId)
    : decision?.action === 'declare'
      ? decision.halfSuitId
      : null;

  const selectedSuitPriority = selectedHalfSuitId
    ? suitPriorities.find((entry) => entry.halfSuitId === selectedHalfSuitId) ?? null
    : null;

  const selectedHalfSuitKnowledge = _summarizeHalfSuitKnowledge(gs, botId, selectedHalfSuitId);

  const knownCardState = decision?.action === 'ask'
    ? _summarizeKnownCardState(gs, botId, decision.cardId)
    : null;

  const selectedTargetPriority = decision?.action === 'ask' && decision.targetId && selectedHalfSuitId
    ? _getOpponentAskPriority(gs, botId, decision.targetId, selectedHalfSuitId, currentMoveIndex)
    : null;

  const actionReason = decision?.action === 'ask'
    ? decision.botAskNarration?.reason ?? 'none'
    : decision?.action === 'declare'
      ? 'team_declare'
      : decision?.action ?? 'unknown';

  const assignmentCoverage = decision?.action === 'declare' && decision.assignment
    ? Object.keys(decision.assignment).length
    : null;

  const context = {
    moveIndex: currentMoveIndex,
    undeclaredHalfSuitCount: undeclaredHalfSuits.length,
    botHandCount: getCardCount(gs, botId),
    validOpponentCount: validOpponents.length,
    liveTeamSignalSuits: topSuitPriorities.filter((entry) => entry.teamSignalStrength > 0).length,
    blockedTeammateChaseCount: blockedChases.length,
    topSuitPriorityCount: topSuitPriorities.length,
  };

  const reasonEvidence = _buildReasonEvidence(
    actionReason,
    selectedSuitPriority,
    selectedHalfSuitKnowledge,
    knownCardState,
  ...
  return {
    ...
    context,
    selected: {
      halfSuitId: selectedHalfSuitId,
      suitPriority: selectedSuitPriority,
      halfSuitKnowledge: selectedHalfSuitKnowledge,
      knownCardState,
      selectedTargetPriority,
      teammateSignalSource: decision?.botAskNarration?.sourcePlayerId ? 'teammate_signal_present' : 'none',
      focusCardHint: decision?.botAskNarration?.focusCardId ? 'focus_card_present' : 'none',
      assignmentCoverage,
    },
    reasonEvidence,
    topSuitPriorities,
    blockedTeammateChases: blockedChases,
  };
}
